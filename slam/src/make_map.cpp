...
*/
    ...
    log_odds[idx.x][idx.y] = new_val;

    int new_state = get_state(idx);
    occ_grid.data[idx.y*width+idx.x] = new_state;

    if (old_state != OCCUPIED && new_state == OCCUPIED)
    {
        add_inflation(idx, true);
    }
    else if (old_state == OCCUPIED && new_state != OCCUPIED)
    {
        add_inflation(idx, false);
    }
}

int LidarMap::get_state(const MapIdx &idx)
{
    if(log_odds[idx.x][idx.y] >= L_THRESH)
    {
        return OCCUPIED;
    }
    else if (log_odds[idx.x][idx.y] <= -L_THRESH)
    {
        return inflation[idx.x][idx.y].empty() ? FREE : INFLATED;
    }
    else
    {
        return inflation[idx.x][idx.y].empty() ? UNKNOWN : INFLATED;
    }
}
    ...
    for(int i=0;i<mask_inflation.size();i++)
    {
        ...
        if(idx_in_map(idx))
        {
            if(occupied)
            {
                ...
                occ_grid.data[width*idx.y+idx.x] = get_state(idx);
            }
            else
            {
